...
export class SpindlerBattery implements Battery {
   ...
   * @returns True if the battery needs service (more than 2 years since last service), false otherwise.
   */
  needsService(lastServiceDate: Date): boolean {
    const serviceThresholdDate = new Date(lastServiceDate);
    serviceThresholdDate.setFullYear(serviceThresholdDate.getFullYear() + 2);
